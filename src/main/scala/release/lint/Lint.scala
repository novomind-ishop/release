...
object Lint {
  ...
  def dockerfiles(rootFile: File): Seq[Path] = {
    FileUtils.walk(rootFile).par.filter(_.endsWith("Dockerfile")).seq.toSeq
  }
    ...
    try {
      ...
      if (files == null || files.isEmpty) {
        ...
        val remoteRefs = sgit.lsRemote()
          ...
          .filterNot(_.refName == "refs/meta/config")
        ...
        remoteRefs.foreach(ref => {
          ...
          // TODO promote removal? git push origin :refs/for/master
        })
        ...
        if (pompom.isDefined || sbt.isDefined || mockMod.isDefined) {
          ...
          if (modTry.isSuccess) {
            ...
            usedLintSkips = usedLintSkips ++ VersionSkew.skewResultOf(mod, None, warnExit, errorExit, Some(out), workOpts,
            ...
            try {
              ...
              val updateResult = if (resultTry.isSuccess) {
                ...
                if (opts.lintOpts.skips.contains(fiCodeSkipBinaryRepo)) {
                  ...
                  val str1 = Util.generateSymbols(1000, 72)
