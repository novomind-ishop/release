...
object FeatureBranch {
  def work(workDirFile: File, sys: Term.Sys, sgit: Sgit, branch: String, rebaseFn: () => Unit,
    ...
    val featureName =
      PomMod.checkNoSlashesNotEmptyNoZeros(Term.readFrom(sys, "Enter the feature name (without 'feature/' prefix)", "", opts))
    ...
    def checkFeatureBranch(sys: Term.Sys): Unit = {
      if (sgit.listBranchNamesLocal().contains("feature")) {
        val changes = Term.readFromOneOfYesNo(sys,
          "You have a local branch with name 'feature'. " +
    ...
    sgit.doCommitPomXmlsAnd(
      """[%s] prepare - %s
        ...
        msgs,
        config.signedOfBy(), Starter.sign(sgit), toolSh1),
      mod.depTreeFilenameList()
    )
