...
class RepoTest extends AssertionsForJUnit {
  ...
  def testExistsGav(): Unit = {
    ...
    Assume.assumeTrue(repo.isReachable().online)
    @nowarn
    ...
    println(repo.getRelocationOf("poi", "poi", "1.6.5"))
    return
  ...
  def testNewerAndPrevVersionsOf_SNAPSHOT(): Unit = {
    Assert.assertEquals(
      Seq("32.1.2-android", "33-SNAPSHOT"),
      Repo.convertNewerAndPrefVersions(
        "com.google.guava",
        "guava",
        "33-SNAPSHOT",
        k =>
  ...
  def testNewerAndPrevVersionsOf(): Unit = {
    Assert.assertEquals(
      Seq("32.1.1-jre", "32.1.1-android", "32.1.2-jre", "32.1.2-android"),
      Repo.convertNewerAndPrefVersions(
        "com.google.guava",
        "guava",
        "32.1.2-jre",
  ...
  def testParseArtifactoryDate(): Unit = {
    val input =
      "<a href=\"jfrog-20240515.115211-1.data\">jfrog-20240515.115211-1.data</a>                         15-May-2024 11:59  268 bytes"
    val out = Repo.extractDate(input)
